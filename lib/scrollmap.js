  ...
  render() {
    ...
    for (let [name, layer] of this.layers) {
      ...
      for (let i = 0; i < layer.items.length; i++) {
        ...
        const height = item.piz ? (item.piz - item.pix) * scale + markerHeight : markerHeight;
        const stl = `transform:translateY(${item.pix * scale}px);height:${height}px`;
        ...
        items.push(
          <div
            ...
            style={stl}
            onclick={() => this.scrollToRow(item.row)}
          />,
        );
      }
    }
